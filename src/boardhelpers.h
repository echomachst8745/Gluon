...
constexpr std::uint8_t WHITE_KING_SIDE_CASTLE  = 1 << 3;
constexpr std::uint8_t WHITE_QUEEN_SIDE_CASTLE = 1 << 2;
constexpr std::uint8_t BLACK_KING_SIDE_CASTLE  = 1 << 1;
constexpr std::uint8_t BLACK_QUEEN_SIDE_CASTLE = 1 << 0;
...
constexpr int FileRankToSquare(int file, int rank)
{
    ...
    {
        throw std::invalid_argument("Invalid file or rank: [" + std::to_string(file) + ", " + std::to_string(rank) + "]");
    }
